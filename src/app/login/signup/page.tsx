            ...
            <div className="w-full max-w-md bg-[#121318]/90 backdrop-blur-xl p-8 rounded-2xl border border-[#232631] shadow-[0_10px_40px_rgba(0,0,0,0.5)] space-y-6 text-zinc-100 relative overflow-hidden">
                ...
                <div className="text-center space-y-2 relative z-10">
                    ...
                    <p className="text-xs text-zinc-400 max-w-xs mx-auto">
                        Rejoins la communauté et sauvegarde ta progression sur le site.
                    </p>
                </div>

                {/* Notice d'erreur ou d'information */}
                {searchParams.error && (
                    <Notice type="error" title="Erreur d'inscription" message={searchParams.error} />
                )}
                {searchParams.message && (
                    <Notice type="success" title="Compte créé" message={searchParams.message} />
                )}
                ...
                <form className="space-y-4 relative z-10">
                    <div className="space-y-3.5">
                        <div>
                            <label htmlFor="username" className="block text-xs font-bold text-zinc-400 uppercase tracking-wider mb-1.5 flex items-center gap-1.5">
                                <User className="w-3.5 h-3.5 text-amber-400" />
                                Pseudo Minecraft / Site
                            </label>
                            <input
                                id="username"
                                name="username"
